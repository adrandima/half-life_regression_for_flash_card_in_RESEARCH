...
if __name__ == "__main__":
    ...
    input_file='./settles.acl16.learning_traces.13m.csv.gz'

    max_lines=None

    # read data set
    trainset, testset = read_data(input_file, method, b, l, max_lines)
    sys.stderr.write('|train| = %d\n' % len(trainset))
    sys.stderr.write('|test|  = %d\n' % len(testset))

    # train model & print preliminary evaluation info
    model = SpacedRepetitionModel(method=method, omit_h_term=t)
    model.train(trainset)
    model.eval(testset, 'test')

    # write out model weights and predictions
    filebits = [method] + \
        [k for k, v in sorted(vars().iteritems()) if v is True] + \
        [os.path.splitext(os.path.basename(input_file).replace('.gz', ''))[0]]
    if max_lines is not None:
        filebits.append(str(max_lines))
